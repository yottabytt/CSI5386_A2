'''
Build a sentence encoding-based method for semantic relatedness
'''
import theano
...
def prepare_data(seqs_x, seqs_y, labels, maxlen=None):
    ...
    l = numpy.zeros((n_samples,)).astype('float32')
...
def ptime_prepare_data(seqs_x, seqs_y, maxlen=None):
    # x: a list of sentences
    lengths_x = [len(s) for s in seqs_x]
    lengths_y = [len(s) for s in seqs_y]
...
def init_params(options, worddicts):
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_layer_output',
                                nin=options['dim'], nout=1, ortho=False)
...
def build_model(tparams, options):
    ...
    x1, x1_mask, x2, x2_mask, y: theano variables
        Represent the input, input binary mask, output for a single batch
    opt_ret: OrderedDict
    ...
    #pooling layers
    rep1 = (ctx1 * x1_mask[:, :, None]).sum(0) / x1_mask.sum(0)[:, None]
    rep2 = (ctx2 * x2_mask[:, :, None]).sum(0) / x2_mask.sum(0)[:, None]

    # concatenating the pooled layers with extra abs, product representation
    logit = concatenate([rep1, rep2, abs(rep1-rep2), rep1*rep2], axis=1)
    ...
    if options['use_dropout']:
        logit = dropout_layer(logit, use_noise, trng)
    nw_out = get_layer('ff')[1](tparams, logit, options,
                               prefix='ff_layer_output', activ='tanh')


    # transformation of [-1,1] outputs to [1,5] 
    pred_out = (( nw_out + 1 ) * 2.0) + 1


    #MSE cost
    cost = tensor.sum(tensor.pow(pred_out.T-y,2)) / (n_samples)


    f_pred = theano.function([x1, x1_mask, x2, x2_mask],
    pred_out, name='f_pred')
...
def ptime_predict(f_pred, ptime_prepare_data, options, iterator, verbose=False):
    """
    Just compute the accuracy
    f_pred: Theano fct computing the prediction
    prepare_data: usual prepare_data for that dataset.
    ...
    for x1, x2, y in iterator:
        n_done += len(x1)
        x1, x1_mask, x2, x2_mask = ptime_prepare_data(x1, x2)
        preds = f_pred(x1, x1_mask, x2, x2_mask)
        ...
        for x1, x2, y in train:
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                ...
                train_cost = pred_probs(f_log_probs, prepare_data,
                                       model_options, train_valid).mean()
...
def load_and_predict(load_from,
    n_words,
    valid_batch_size,
    test_datasets,
    dictionary,
    ...
    all_preds = ptime_predict(f_pred, ptime_prepare_data, model_options, test)
